...
unordered_map<u_int,pair<int,int>> remote_port;
...
int main(int argc, const char * argv[]) {
    if(argc!=3) {
        cout<<"Arguments Fault. Expected 3.\n";
        cout<<"Usage:<program> <data_dir(pcap+iotList)> <out_dir>\n";
        exit(1);
    }
    ...
    //char* DATADIR=new char[strlen(argv[1])+1];
    ...
    vector<string> data_files=getFilenames(DATADIR,"pcap");
    ...
    int sim_count=0;
    for (unordered_map<u_int,pair<int,int>>::iterator it=remote_port.begin();
    ...
    //int sim_count=0;
    for (unordered_map<string,int>::iterator it=domain.begin();it!=domain.end(); ++it) {
    ...
    for(unordered_map<string,int>::iterator it=cipher_suite.begin();
    ...
    if (ntohs(ethernetHeader->ether_type) == ETHERTYPE_IP) {
        ...
        if (ipHeader->ip_p == IPPROTO_TCP) {
            ...
            if(remote_port.count(destPort)==0) { //new port
                remote_port.insert(pair<u_int,pair<int,int>>(destPort,pair<int,int>(1,0)));
            }
            else { //found port
            ...
            if(destPort==443 && data[0]==0x16 && data[5]==0x01) {
                ...
                if(cipher_suite.count(strTmp)==0) {//not exist
                    cipher_suite.insert(pair<string,int>(strTmp,1));
                }
                else {//found
                    ++cipher_suite[strTmp];
                }
        ...
        if (ipHeader->ip_p == IPPROTO_UDP) {
            ...
            if(remote_port.count(destPort)==0) { //new port
                remote_port.insert(pair<u_int,pair<int,int>>(destPort,pair<int,int>(0,1)));
            }
            else { //found port
            ...
            if(sourcePort==53 || destPort==53) {
                ...
                if(domain.count(dmTmp)==0) { //new domain
                    domain.insert(pair<string,int>(dmTmp,1));
                }
                else { //found domain
                    ++domain[dmTmp];
                }
